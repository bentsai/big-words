...
// Maps each front matter key to its list of allowed values.
// To add a new option, add an entry here and a corresponding default above.
const VALID = { theme: THEMES, font: FONTS, transition: TRANSITIONS };
...
function parseFrontMatter(raw) {
  ...
  for (const line of raw.split('\n')) {
    ...
    if (VALID[key]?.includes(value)) config[key] = value;
  }
  return config;
}
